namespace Shop.BLL.Services
{
    ...
    public class AlbumService : BaseService, IAlbumService
    {
        ...
        /// </returns>
        public AlbumDetailsViewModel GetAlbumDetails(
            int id,
            ...
            int? userId = null)
        {
            ...
            if (userId != null)
            {
                using (var repository = this.Factory.GetOrderAlbumRepository())
                {
                    albumViewModel.IsOrdered =
                        repository.FirstOrDefault(o => o.Cart.UserId == userId && o.AlbumId == albumViewModel.Id)
                        != null;
                }

                using (var repository = this.Factory.GetPurchasedAlbumRepository())
                {
        ...
        /// </returns>
        public ICollection<AlbumViewModel> GetAlbumsWithPrice(
            int? currencyCode = null,
            int? priceLevel = null,
            int? userId = null)
        {
        ...
        /// </returns>
        public AlbumTracksListViewModel GetTracksList(
            int albumId,
            int? currencyCode = null,
            int? priceLevel = null,
            int? userId = null)
        ...
        /// <returns>
        /// </returns>
        public AlbumTracksListViewModel GetTracksToAdd(
            int albumId,
            int? currencyCode = null,
            int? priceLevel = null,
            int? userId = null)
            ...
            using (var repository = this.Factory.GetTrackRepository())
            {
                ...
                else
                {
                    tracks =
                        repository.GetAll(
                            t =>
                                t.ArtistId == albumTracksListViewModel.Artist.Id
                                && (t.OwnerId == null || t.OwnerId == albumTracksListViewModel.OwnerId)
                                && (!t.Albums.Any() || t.Albums.All(r => r.AlbumId != albumId)),
                            t => t.Artist);
                }
